"""
RCAEval V4.4 模型：移除因果模块（消融实验）

基于 V3_host，完全移除因果模块，验证其贡献。

动机：
- 因果矩阵在 RCAEval 上总是收敛到单位矩阵（没学到东西）
- 移除因果模块 = 减少参数 = 可能减少过拟合
...
class MultiModalV4_4_NoCausal_RCAEval(nn.Module):
    """
    RCAEval V4.4 模型：移除因果模块
    ...
    def __init__(self, config: V4Config, adjacency_matrix: torch.Tensor = None):
        ...
        # 3. Temporal Attention (MHA) - 无因果模块
...
__all__ = ['MultiModalV4_4_NoCausal_RCAEval']
