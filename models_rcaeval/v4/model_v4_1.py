"""
RCAEval V4.1 模型：跨模态融合增强

基于 V3_host，核心改进：
1. 新增 CrossModalFusion 模块，让 metrics 主动查询 logs 和 traces
2. 移除因果模块（在 RCAEval 上无效）
3. 优化特征融合流程

架构：
1. Embedding: Metric/Log/Trace → D维 + PE
2. CrossModalFusion: Metric 查询 Log/Trace（V4.1 核心创新）
3. Spatial Attention: GATv2Conv（空间建模）
...
from models_rcaeval.v4.cross_modal_fusion import CrossModalFusion
...
class ReconstructionHead(nn.Module):
    ...
    def __init__(self, embed_dim: int, metric_dim: int, log_dim: int, trace_dim: int):
        super().__init__()
        # embed_dim 是 2D（node_feat 的维度）
        # 分别重构 metric 和 log
        self.metric_head = nn.Linear(embed_dim // 2, metric_dim)  # 从前半部分重构 metric
        self.log_head = nn.Linear(embed_dim // 2, log_dim)        # 从后半部分重构 log
        self.trace_head = nn.Linear(embed_dim // 2, trace_dim)    # edge_feat 是 D 维度
    
    def forward(self, node_feat, edge_feat):
        # node_feat: (B, T, N, 2D)
        # edge_feat: (B, T, E, D)
        D = node_feat.shape[-1] // 2
        metric_feat = node_feat[..., :D]   # 前半部分
        log_feat = node_feat[..., D:]      # 后半部分
...
class MultiModalV4_1_RCAEval(nn.Module):
    """
    RCAEval V4.1 模型：跨模态融合增强
    
    核心创新：CrossModalFusion 让 metrics 主动查询 logs 和 traces
    """
    ...
    def __init__(self, config: V4Config, adjacency_matrix: torch.Tensor = None):
        ...
        # 2. CrossModalFusion（V4.1 核心创新）
        # 修复：融合后与 log 拼接，保持 2D 维度
        if config.use_cross_modal_fusion:
            self.cross_modal_fusion = CrossModalFusion(
                metric_dim=config.embedding_dim,
                log_dim=config.embedding_dim,
                trace_dim=config.embedding_dim,
        ...
        # 3. Spatial Attention (GATv2)
        # 修复：始终保持 2D 维度（融合后的 metric + log）
        spatial_dim = config.embedding_dim * 2  # 始终是 2D
    ...
    def _print_info(self):
        ...
        print(f"V4.1 模型初始化完成（跨模态融合增强版）:")
        print(f"  嵌入维度: {self.config.embedding_dim}")
        print(f"  跨模态融合: {'✅ 启用' if self.config.use_cross_modal_fusion else '❌ 禁用'}")
        if self.config.use_cross_modal_fusion:
            print(f"    - 注意力头数: {self.config.cross_modal_heads}")
            print(f"    - 融合权重: α={self.config.fusion_alpha}, β={self.config.fusion_beta}")
    ...
    def forward(
        ...
        # ========== Step 2: CrossModalFusion（V4.1 核心）==========
        if self.cross_modal_fusion is not None:
            # 跨模态融合：metric 查询 log 和 trace，增强 metric 特征
            enhanced_metric = self.cross_modal_fusion(metric_emb, log_emb, trace_emb)  # (B, T, N, D)
            # 修复：与 log 拼接，保持 2D 维度（与 V3 一致）
            node_feat = torch.cat([enhanced_metric, log_emb], dim=-1)  # (B, T, N, 2D)
        else:
            # 回退到 V3 的简单拼接
            node_feat = torch.cat([metric_emb, log_emb], dim=-1)  # (B, T, N, 2D)
        
        # ========== Step 3: Spatial Attention (GATv2) ==========
...
__all__ = ['MultiModalV4_1_RCAEval']
