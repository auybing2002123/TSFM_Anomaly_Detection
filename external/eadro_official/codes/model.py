...
class ConvNet(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_sizes, dilation=2, dev="cpu", dropout=0.0):
        ...
        for i in range(len(kernel_sizes)):
            ...
            layers += [nn.Conv1d(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size, padding=padding), 
                       nn.BatchNorm1d(out_channels), nn.ReLU(), nn.Dropout(dropout), Chomp1d(padding)]
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        weighted_sum = torch.bmm(atten_weight.transpose(1, 2), x).squeeze(1)
        return weighted_sum
...
class TraceModel(nn.Module):
    def __init__(self, device='cpu', trace_hiddens=[20, 50], trace_kernel_sizes=[3, 3], self_attn=False, chunk_lenth=None, **kwargs):
        ...
        trace_dropout = kwargs["trace_dropout"] if "trace_dropout" in kwargs else 0.0
...
class MetricModel(nn.Module):
    def __init__(self, metric_num, device='cpu', metric_hiddens=[64, 128], metric_kernel_sizes=[3, 3], self_attn=False, chunk_lenth=None, **kwargs):
        ...
        metric_dropout = kwargs["metric_dropout"] if "metric_dropout" in kwargs else 0.0
