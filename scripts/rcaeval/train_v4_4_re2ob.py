"""
RCAEval RE2-OB V4.4 无因果模块模型训练脚本（消融实验）

核心改进：
- 完全移除因果模块，验证其贡献
- 因果矩阵在 RCAEval 上总是收敛到单位矩阵，可能是冗余的

用法:
    cd code/TSFM_Anomaly_Detection
    python scripts/rcaeval/train_v4_4_re2ob.py --epochs 40
"""
import argparse
...
from models_rcaeval.v4.model_v4_4 import MultiModalV4_4_NoCausal_RCAEval
from data_rcaeval.dataset_loader import create_rcaeval_dataloaders


def parse_args():
    parser = argparse.ArgumentParser(description='Train V4.4 No-Causal Model on RCAEval RE2-OB')
...
def main():
    ...
    # 创建模型
    model = MultiModalV4_4_NoCausal_RCAEval(config, adjacency_matrix).to(device)
    ...
    if args.save_dir:
        ...
        save_dir = PROJECT_ROOT / 'checkpoints/rcaeval/v4_4_nocausal_re2ob'
    ...
    print(f"\n对比 V3 (有因果): Val F1=0.8479, Test F1=0.8135")
    print(f"因果模块贡献: Test F1 差距 = {test_metrics['f1'] - 0.8135:.4f}")
