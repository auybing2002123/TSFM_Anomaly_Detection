"""
V6 RCAEval 训练脚本: GPT-2 预测主干 + 多模态编码

用法:
    python scripts/rcaeval/train_v6_re2ob.py --epochs 40
    python scripts/rcaeval/train_v6_re2ob.py --use-lora --lora-rank 4
"""
import argparse
import torch
...
from data_rcaeval.dataset_loader import load_rcaeval_temporal_split


def parse_args():
    parser = argparse.ArgumentParser(description='V6 RCAEval: GPT-2 预测主干')
    ...
    parser.add_argument('--save-dir', type=str,
                        default='checkpoints/rcaeval/v6_re2ob')
...
def main():
    ...
    splits = load_rcaeval_temporal_split(args.data_file)
    ...
    print(f"服务数: {metadata['num_services']}, "
          f"每服务指标: {metadata['metrics_per_service']}")
