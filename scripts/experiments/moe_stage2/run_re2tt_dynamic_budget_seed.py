...
OFFLINE = PROJECT_ROOT / "scripts" / "experiments" / "moe_stage2" / "offline_dynamic_budget_re2tt.py"
RUNNER = PROJECT_ROOT / "scripts" / "experiments" / "moe_stage2" / "online_service_aware_moe_re2tt_runner.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run one RE2-TT dynamic-budget seed with validation-selected tau.")
    ...
    parser.add_argument("--output-dir", type=str, default="results/experiments/moe_stage2/dynamic_budget_re2tt_final")
...
def offline_path(out_dir: Path, split: str, tau: float) -> Path:
    return out_dir / f"offline_dynamic_budget_re2tt_{split}_tau{tau:.2f}.json"


def find_latest_replay_summary(out_dir: Path, before: set[Path]) -> Path:
    candidates = set(out_dir.glob("re2tt_service_aware_moe_test_*_summary.json")) - before
    if not candidates:
...
def main() -> int:
    ...
    best_key = None
    for tau in tau_grid:
        path = offline_path(offline_out, "val", tau)
        if not path.exists():
            run(
                [
                    str(PYTHON),
    ...
    test_path = offline_path(offline_out, "test", best_tau)
    ...
    test_offline = read_json(test_path)

    before = set(replay_out.glob("re2tt_service_aware_moe_test_*_summary.json"))
    replay_cmd = [
        str(PYTHON),
        str(RUNNER),
        "--split",
    ...
    if args.prefetch:
        replay_cmd.append("--prefetch")
    if args.pin_memory:
        replay_cmd.append("--pin-memory")
    run(replay_cmd)
    ...
    report = {
        ...
        "test_replay": {
            ...
            "detection_metrics": replay["detection_metrics"],
            "router_budget": replay["router_budget"],
        },
    }
    report_path = base_out / f"re2tt_dynamic_budget_seed{args.seed}_report.json"
    write_json(report_path, report)
    metrics = test_offline["detection_metrics"]
    replay_budget = replay["router_budget"]
    print("=" * 72)
    print(f"RE2-TT dynamic budget seed={args.seed}")
    print("=" * 72)
