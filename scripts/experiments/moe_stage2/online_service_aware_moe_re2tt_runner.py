...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage-2 service-aware MoE 在线 replay runner（RE2-TT）")
    ...
    parser.add_argument("--output-dir", type=str, default="results/experiments/moe_stage2/realtime")
...
def load_runtime_bundle_service_aware_re2tt(
    split: str,
    checkpoint: str,
    data_dir: str,
    device: torch.device,
    ...
    if metadata.get("adjacency_matrix"):
        ...
        base_adjacency = ((adj_raw + adj_2hop) > 0).float()
        base_adjacency.fill_diagonal_(0)
    else:
        base_adjacency = None

    checkpoint_data = torch.load(checkpoint_path, map_location=device)
    config = ServiceAwareMoERCAEvalConfig(**checkpoint_data["config"])
    adjacency_matrix = build_adjacency_from_mode(
        base_adjacency,
    ...
    return RuntimeBundle(
        dataset_name="re2tt_service_aware_moe",
        split_name=split,
        model=model,
        dataset=dataset,
...
def main() -> int:
    ...
    bundle = load_runtime_bundle_service_aware_re2tt(args.split, args.checkpoint, args.data_dir, device)
    ...
    run_id = f"re2tt_service_aware_moe_{args.split}_{timestamp_tag()}"

    print(f"Loaded {len(bundle.dataset)} samples from re2tt/{args.split}")
    ...
    print("\n" + "=" * 72)
    print("Service-aware MoE Realtime Summary (RE2-TT)")
    print("=" * 72)
