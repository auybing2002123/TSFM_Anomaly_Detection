...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Offline RE2-TT dynamic-budget validation/test evaluation.")
    ...
    parser.add_argument("--output-dir", type=str, default="results/experiments/moe_stage2/dynamic_budget_re2tt_offline")
...
def load_model(args: argparse.Namespace, device: torch.device):
    ...
    model = MultiModalServiceAwareMoE_RCAEval(config, adjacency_tensor).to(device)
...
def main() -> int:
    ...
    with torch.no_grad():
        for batch in dataloader:
            data_node = batch["metrics"].float().to(device)
            data_log = batch["logs"].float().to(device)
            data_edge = batch["traces"].float().to(device)
    ...
    summary = {
        "experiment": "offline_dynamic_budget_re2tt",
    ...
    path = out_dir / f"offline_dynamic_budget_re2tt_{args.split}_tau{args.tau:.2f}.json"
