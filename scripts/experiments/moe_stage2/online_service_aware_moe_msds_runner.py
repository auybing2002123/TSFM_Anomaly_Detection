...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage-2 service-aware MoE 在线 replay runner（MSDS）")
...
def load_runtime_bundle_service_aware_msds(
    split: str,
    checkpoint: str,
    data_dir: str,
    device: torch.device,
    ...
    config = ServiceAwareMoEConfig(**checkpoint_data["config"])
    ...
    return RuntimeBundle(
        dataset_name="msds_service_aware_moe",
        split_name=split,
        model=model,
        dataset=dataset,
...
def main() -> int:
    ...
    run_id = f"msds_service_aware_moe_{args.split}_{timestamp_tag()}"
    ...
    print("\n" + "=" * 72)
    print("Service-aware MoE Realtime Summary (MSDS)")
    print("=" * 72)
