...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate RCA direction-2 case aggregation strategies")
...
def build_model_from_checkpoint(
    ...
    device: torch.device,
) -> tuple[MultiModalV6DisentangleRCAEval, Dict]:
    checkpoint = torch.load(checkpoint_path, map_location=device)
    checkpoint_config = checkpoint["config"]
    config = V6DisentangleRCAEvalConfig(**checkpoint_config)
...
def collect_split_outputs(
    ...
    case_names: List[str] = []
    disentangle_scores: List[np.ndarray] = []
    root_scores: List[np.ndarray] = []
    victim_scores: List[np.ndarray] = []
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            disentangle_scores.append(outputs["disentangle_probs"].cpu().numpy())
            root_scores.append(outputs["root_probs"].cpu().numpy())
            victim_scores.append(outputs["victim_probs"].cpu().numpy())
            anomaly_scores.append(outputs["anomaly_probs"][..., 1].cpu().numpy())
            labels.append(gt_real.cpu().numpy())

    return {
        "case_names": case_names,
        "disentangle_scores": np.concatenate(disentangle_scores, axis=0),
        "root_scores": np.concatenate(root_scores, axis=0),
        "victim_scores": np.concatenate(victim_scores, axis=0),
...
def evaluate_strategies_for_split(
    ...
    for strategy in strategies:
        disentangle_payload = aggregate_case_scores(
            ...
            split_outputs["disentangle_scores"],
    ...
    for split_name in ("val", "test"):
        ...
        for strategy in args.strategies:
            ...
            print(
                ...
                f"disentangle[{format_service_rca_metrics(metrics['disentangle'])}] "
                f"root_only[{format_service_rca_metrics(metrics['root_only'])}] "
