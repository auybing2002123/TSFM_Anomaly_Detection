...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate an Eadro-SN checkpoint with richer diagnostics")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--data-dir",
        type=str,
        default="data_eadro/processed/sn_lazy",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional path to save the diagnostic report",
    )
...
def collect_outputs(
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            abnormal_labels = (gt_cls[..., 1] + gt_cls[..., 2] > 0).detach().cpu().numpy().astype(np.int64)
...
def main() -> None:
    ...
    label_mode = ckpt_args.get("label_mode", "root")
