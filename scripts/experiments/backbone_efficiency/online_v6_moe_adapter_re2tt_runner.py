...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="V6 MoE-adapter-light 在线 replay runner（RE2-TT）")
...
def load_runtime_bundle_moe_re2tt(
    split: str,
    checkpoint: str,
    data_dir: str,
    device: torch.device,
    ...
    config = V6MoEAdapterRCAEvalConfig(**checkpoint_data["config"])
    ...
    return RuntimeBundle(
        dataset_name="re2tt_moe_adapter",
        split_name=split,
        model=model,
        dataset=dataset,
...
def main() -> int:
    ...
    bundle = load_runtime_bundle_moe_re2tt(args.split, args.checkpoint, args.data_dir, device)
    ...
    run_id = f"re2tt_moe_adapter_{args.split}_{timestamp_tag()}"
    ...
    print("\n" + "=" * 72)
    print("MoE-adapter Realtime Summary (RE2-TT)")
    print("=" * 72)
