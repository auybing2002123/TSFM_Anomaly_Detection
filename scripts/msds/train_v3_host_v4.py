"""
V3_host.4 训练脚本：V3_host + Masked Temporal Attention

用法:
    conda activate paper_env & python -B scripts/msds/train_v3_host_v4.py --epochs 30

改进点:
    - 在 V3_host 基础上添加因果掩码时序注意力
    - t=5 只能看 t=1,2,3,4，不能看未来
    - 这是自回归/因果建模的标准做法
"""
import argparse
import torch
...
from models_msds.v3.model_host_causal_v4 import MultiModalV3HostCausalV4_MSDS
from data_msds.dataset_loader import load_msds_temporal_split


def parse_args():
    parser = argparse.ArgumentParser(description='Train V3_host.4 Model (+ Masked Temporal)')
...
def main():
    ...
    # 创建模型
    model = MultiModalV3HostCausalV4_MSDS(config).to(device)
    ...
    best_f1 = 0.0
    patience_counter = 0
    save_dir = Path('checkpoints/msds/v3_host_v4')
    save_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\nStarting training for {args.epochs} epochs...")
    print(f"Model: V3_host.4 (V3_host + Masked Temporal)")
