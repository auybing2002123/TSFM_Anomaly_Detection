"""
V4.4 训练脚本：Spatial Attention → Graph Transformer

用法:
    python scripts/msds/train_v4_4.py --epochs 20

与 V3 的区别：
- Spatial 模块从 GATv2 换成 Graph Transformer
- 添加 Random Walk 位置编码
- 全局注意力 + 边特征注入
"""
import argparse
...
from models_msds.v4.model_v4_4 import MultiModalV4_4_GraphTransformer_MSDS
from models_msds.v3.config import V3Config


def parse_args():
    parser = argparse.ArgumentParser(description='V4.4 训练 (Graph Transformer)')
...
def main():
    ...
    # 创建模型
    print("\n创建 V4.4 模型 (Graph Transformer)...")
    model = MultiModalV4_4_GraphTransformer_MSDS(config).to(device)
    ...
    save_dir = Path('checkpoints/msds/v4_4')
