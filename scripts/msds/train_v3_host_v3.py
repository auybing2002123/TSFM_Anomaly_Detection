"""
V3_host.3 训练脚本：V3_host + 双向空间建模

用法:
    conda activate paper_env & python -B scripts/msds/train_v3_host_v3.py --epochs 30

改进点:
    - 在 V3_host 基础上添加双向空间建模
    - node ↔ edge 双向信息传递
    - node → edge: 用 trace2pod.T 把 node 信息聚合到 edge
    - edge → node: GATv2 + trace2pod 把 edge 信息聚合到 node
"""
import argparse
import torch
...
from models_msds.v3.model_host_causal_v3 import MultiModalV3HostCausalV3_MSDS
from data_msds.dataset_loader import load_msds_temporal_split


def parse_args():
    parser = argparse.ArgumentParser(description='Train V3_host.3 Model (+ Bidirectional Spatial)')
...
def main():
    ...
    # 创建模型
    model = MultiModalV3HostCausalV3_MSDS(config).to(device)
    ...
    best_f1 = 0.0
    patience_counter = 0
    save_dir = Path('checkpoints/msds/v3_host_v3')
    save_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\nStarting training for {args.epochs} epochs...")
    print(f"Model: V3_host.3 (V3_host + Bidirectional Spatial)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 获取学习到的门控权重
        node_gate = torch.sigmoid(model.bidirectional_spatial_layers[0].node_gate).item()
        edge_gate = torch.sigmoid(model.bidirectional_spatial_layers[0].edge_gate).item()
        ...
        print(f"\nEpoch {epoch}: "
              ...
              f"gates=(n:{node_gate:.2f}, e:{edge_gate:.2f})")
    ...
    print(f"      Learned gates: node={checkpoint['node_gate']:.3f}, edge={checkpoint['edge_gate']:.3f}")
