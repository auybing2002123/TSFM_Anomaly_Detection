"""
V3_host.1 训练脚本：V3_host + Decoder 结构

用法:
    conda activate paper_env & python -B scripts/msds/train_v3_host_v1.py --epochs 30

改进点:
    - 在 V3_host 基础上添加 Decoder 结构
    - Decoder 使用 Masked MHA（因果掩码）+ Cross-Attention
"""
import argparse
import torch
...
from models_msds.v3.model_host_causal_v1 import MultiModalV3HostCausalV1_MSDS
from data_msds.dataset_loader import load_msds_temporal_split


def parse_args():
    parser = argparse.ArgumentParser(description='Train V3_host.1 Model (+ Decoder)')
    ...
    parser.add_argument('--num-decoder-layers', type=int, default=2,
                        help='Decoder 层数（默认与 Encoder 相同）')
...
def main():
    ...
    # 创建模型
    model = MultiModalV3HostCausalV1_MSDS(config).to(device)
    ...
    best_f1 = 0.0
    patience_counter = 0
    save_dir = Path('checkpoints/msds/v3_host_v1')
    save_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\nStarting training for {args.epochs} epochs...")
    print(f"Model: V3_host.1 (V3_host + Decoder)")
