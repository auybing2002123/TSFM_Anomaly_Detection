...
class ChronosBackbone(nn.Module):
    ...
    def _load_pipeline(self):
        ...
        # Set environment variables to use local cache and disable network
        # 必须在导入 transformers/huggingface_hub 之前设置
        os.environ['HF_HOME'] = str(cache_path)
        os.environ['TRANSFORMERS_CACHE'] = str(cache_path)
        os.environ['HF_DATASETS_CACHE'] = str(cache_path)
        os.environ['HF_HUB_OFFLINE'] = '1'  # 强制离线模式，不联网
        ...
        try:
            self._pipeline = ChronosPipeline.from_pretrained(
                ...
                trust_remote_code=False,  # 禁用远程代码，避免检查 custom_generate
            )
        except Exception as e:
            # 如果本地没有缓存，关闭离线模式重试
            logger.warning(f"Local cache not found, downloading from HuggingFace: {e}")
