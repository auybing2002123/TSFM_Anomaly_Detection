"""
MSDS V3_host.4 模型：V3_host + Masked Temporal Attention

在 V3_host 基础上新增：
1. 因果掩码时序注意力（借鉴 MSTGAD Decoder）
2. t=5 只能看 t=1,2,3,4，不能看未来

架构：
1. Embedding: Metric/Log/Trace → D维 + PE
2. Spatial Attention: GATv2Conv（空间建模）
3. Host Causal Attention: 跨主机因果融合
4. Masked Temporal Attention: 因果掩码 MHA（V3_host.4 核心改进）
5. 检测头: 重构 + 分类
"""
import torch
...
class MaskedTemporalAttentionBlock(nn.Module):
    """
    带因果掩码的时序注意力块
    
    核心思想：
    - 使用下三角掩码，让 t 时刻只能看到 t 及之前的时刻
    - 这是自回归/因果建模的标准做法
    """
    ...
    def _generate_causal_mask(self, seq_len: int, device: torch.device) -> torch.Tensor:
        ...
        mask[i,j] = 0 表示 i 可以看 j
        mask[i,j] = -inf 表示 i 不能看 j
...
class MultiModalV3HostCausalV4_MSDS(nn.Module):
    """
    MSDS V3_host.4 模型：V3_host + Masked Temporal Attention
    ...
    4. Masked Temporal: 因果掩码 MHA（V3_host.4 核心改进）
    5. Detection: 重构 + 分类
    """
    ...
    def __init__(self, config: V3Config, adjacency_matrix: torch.Tensor = None):
        ...
        # 4. Masked Temporal Attention（V3_host.4 核心改进）
        self.masked_temporal_layers = nn.ModuleList([
            MaskedTemporalAttentionBlock(
                embed_dim=config.embedding_dim * 2,
        ...
        # 边的 Masked Temporal Attention
        self.edge_masked_temporal = nn.ModuleList([
            MaskedTemporalAttentionBlock(
    ...
    def _print_info(self):
        ...
        print(f"V3_host.4 模型初始化完成（+ Masked Temporal）:")
        ...
        print(f"  Masked Temporal 层数: {self.config.num_temporal_layers}")
    ...
    def forward(
        ...
        # ========== Step 4: Masked Temporal Attention ==========
        for temporal_layer in self.masked_temporal_layers:
            node_feat = temporal_layer(node_feat)
        
        # 边的 Masked Temporal Attention
        B_e, T_e, E, D_e = edge_emb.shape
        edge_emb_flat = edge_emb.permute(0, 2, 1, 3).reshape(B_e * E, T_e, D_e)
        for edge_layer in self.edge_masked_temporal:
            edge_emb_flat = edge_layer(edge_emb_flat)
        edge_emb = edge_emb_flat.reshape(B_e, E, T_e, D_e).permute(0, 2, 1, 3)
