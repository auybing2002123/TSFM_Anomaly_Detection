...
3. Causal Attention: 跨模态因果融合（V3 新增）
4. Temporal Attention: Multi-head Attention（时序建模）
5. 检测头: 重构 + 分类
"""
import torch
...
from models_msds.v3.causal_attention import CausalAttentionModule, CausalLoss, RootCauseLocator
...
class MultiModalV3_MSDS(nn.Module):
    """
    MSDS V3 模型：因果发现
    ...
    def __init__(self, config: V3Config, adjacency_matrix: torch.Tensor = None):
        ...
        # 2. Spatial Attention (GATv2) - 分别处理 metric 和 log
        ...
        # 3. Causal Attention（V3 核心创新）
        self.causal_attention = CausalAttentionModule(
            embed_dim=config.embedding_dim,
            num_modalities=config.num_modalities,
            hidden_dim=config.causal_hidden_dim,
            num_heads=config.causal_heads,
            dropout=config.causal_dropout,
            init_diag=config.causal_init_diag,
            init_off_diag=config.causal_init_off_diag
        )
        
        # 4. Temporal Attention (MHA) - 处理融合后的特征
        self.temporal_layers = nn.ModuleList([
            TemporalAttentionBlock(
                embed_dim=config.embedding_dim * 3,  # 3 个模态融合
        ...
        self.reconstruction_head = ReconstructionHead(
            config.embedding_dim,  # 从融合特征中分离出各模态
            config.metric_dim,
            config.log_dim,
            config.trace_dim
        )
        ...
        # 特征分离层（从 3D 融合特征分离出 metric 和 log）
        self.feat_separator = nn.Linear(config.embedding_dim * 3, config.embedding_dim * 2)
        ...
        # 7. 根因定位器
        self.root_cause_locator = RootCauseLocator()
    ...
    def _aggregate_trace_to_node(self, edge_feat: torch.Tensor) -> torch.Tensor:
        """
        将边特征聚合到节点（用于因果注意力）
        ...
        # (B*T, E, D) @ (E, N) -> (B*T, D, N) -> (B*T, N, D)
        node_trace = torch.matmul(
            edge_flat.permute(0, 2, 1),
            self.trace2pod
        ).permute(0, 2, 1)
        
        return node_trace.reshape(B, T, N, D)
    ...
    def get_causal_matrix(self) -> torch.Tensor:
        """获取因果矩阵（用于可视化）"""
        return self.causal_attention.causal_matrix.get_causal_matrix()
    ...
    def forward(
        ...
        # 将 trace 聚合到节点
        trace_node = self._aggregate_trace_to_node(edge_emb)  # (B, T, N, D)
        trace_node = self.trace_to_node(trace_node)
        
        # ========== Step 3: Causal Attention（V3 核心）==========
        fused_feat, causal_matrix = self.causal_attention(
            metric_emb, log_emb, trace_node
        )  # (B, T, N, 3D), (3, 3)
        
        # ========== Step 4: Temporal Attention (MHA) ==========
        for temporal_layer in self.temporal_layers:
            fused_feat = temporal_layer(fused_feat)
        ...
        # 从融合特征分离出 metric 和 log 部分
        node_feat = self.feat_separator(fused_feat)  # (B, T, N, 2D)
    ...
    def locate_root_cause(
        ...
        with torch.no_grad():
            ...
            # 计算各模态重构误差（简化版，使用分类概率）
            # 实际应用中应该使用真实的重构误差
            rec_errors = {
                'metric': cls_probs[:, :, 1].mean(dim=0),  # 异常概率作为误差代理
                'log': cls_probs[:, :, 1].mean(dim=0),
                'trace': cls_probs[:, :, 1].mean(dim=0)
            }
            
            return self.root_cause_locator.locate(rec_errors, causal_matrix)
