"""
MSDS V3_host.1 模型：V3_host + Decoder 结构

在 V3_host 基础上新增：
1. Encoder-Decoder 架构（借鉴 MSTGAD）
2. Decoder 使用 Masked MHA（因果掩码，只能看过去）
3. Cross-Attention（Q=decoder, KV=encoder输出）

架构：
1. Embedding: Metric/Log/Trace → D维 + PE
2. Encoder:
   - Spatial Attention: GATv2Conv
   - Host Causal Attention: 跨主机因果融合
   - Temporal Attention: Multi-head Attention
3. Decoder (新增):
   - Masked Temporal Attention: 因果掩码 MHA
...
class DecoderLayer(nn.Module):
    ...
    2. Cross-Attention（Q=decoder, KV=encoder）
    3. FFN
    """
    ...
    def forward(
        self, 
        x: torch.Tensor,           # decoder 输入 (B*N, T, D)
        memory: torch.Tensor,      # encoder 输出 (B*N, T, D)
...
class MultiModalV3HostCausalV1_MSDS(nn.Module):
    """
    MSDS V3_host.1 模型：V3_host + Decoder 结构
    ...
    2. Encoder:
       - Spatial: GATv2 让主机交换信息
       - Host Causal: 跨主机因果注意力
       - Temporal: MHA 学习时序模式
    3. Decoder (新增):
    ...
    def __init__(self, config: V3Config, adjacency_matrix: torch.Tensor = None):
        ...
        # 2. Encoder - Spatial Attention (GATv2)
        ...
        # 3. Encoder - Host Causal Attention
        ...
        # 4. Encoder - Temporal Attention (MHA)
        ...
        # 边的 Encoder 时序注意力
        self.encoder_edge_temporal = nn.ModuleList([
        ...
        # 5. Decoder 层 (新增)
        num_decoder_layers = getattr(config, 'num_decoder_layers', config.num_temporal_layers)
        ...
        # 边的 Decoder 层
        self.decoder_edge_layers = nn.ModuleList([
            DecoderLayer(
    ...
    def _print_info(self):
        ...
        print(f"V3_host.1 模型初始化完成（+ Decoder 结构）:")
        ...
        print(f"  Encoder Temporal 层数: {self.config.num_temporal_layers}")
        print(f"  Decoder 层数: {len(self.decoder_layers)}")
    ...
    def forward(
        ...
        # ========== Step 2: Encoder - Spatial Attention (GATv2) ==========
        for layer in self.spatial_layers:
            node_feat = layer(node_feat, self.adj, edge_matrix)
        
        # ========== Step 3: Encoder - Host Causal Attention ==========
        node_feat, causal_matrix = self.host_causal(node_feat)  # (B, T, N, 2D), (N, N)
        
        # ========== Step 4: Encoder - Temporal Attention (MHA) ==========
        for temporal_layer in self.encoder_temporal_layers:
            node_feat = temporal_layer(node_feat)
        
        # 边的 Encoder 时序注意力
        B_e, T_e, E, D_e = edge_emb.shape
        edge_emb_flat = edge_emb.reshape(B_e * E, T_e, D_e)
        for edge_layer in self.encoder_edge_temporal:
            edge_emb_flat = edge_layer(edge_emb_flat)
        edge_emb = edge_emb_flat.reshape(B_e, T_e, E, D_e)
        
        # Encoder 输出作为 memory
        encoder_node_memory = node_feat.clone()  # (B, T, N, 2D)
        encoder_edge_memory = edge_emb.clone()   # (B, T, E, D)
        
        # ========== Step 5: Decoder ==========
        # 生成因果掩码
        causal_mask = self._generate_causal_mask(T, node_feat.device)
        
        # Node Decoder
        # 重塑为 (B*N, T, 2D) 以便处理
        node_feat_flat = node_feat.permute(0, 2, 1, 3).reshape(B * N, T, -1)
        memory_node_flat = encoder_node_memory.permute(0, 2, 1, 3).reshape(B * N, T, -1)
        
        for decoder_layer in self.decoder_layers:
            node_feat_flat = decoder_layer(node_feat_flat, memory_node_flat, causal_mask)
        
        node_feat = node_feat_flat.reshape(B, N, T, -1).permute(0, 2, 1, 3)  # (B, T, N, 2D)
        
        # Edge Decoder
        edge_emb_flat = edge_emb.reshape(B * E, T, D_e)
        memory_edge_flat = encoder_edge_memory.reshape(B * E, T, D_e)
        
        for decoder_layer in self.decoder_edge_layers:
            edge_emb_flat = decoder_layer(edge_emb_flat, memory_edge_flat, causal_mask)
        
        edge_emb = edge_emb_flat.reshape(B, T, E, D_e)
