"""
MSDS V5 模型：对比学习 + 跨模态交互

基于 V3_host (F1=0.9517) 开发，新增：
1. V5.1 对比学习损失：拉开正常/异常样本的表示距离
2. V5.2 跨模态交互：让 node 和 edge 特征互相增强

架构：
1. Embedding: Metric/Log/Trace → D维 + PE
2. Spatial Attention: GATv2Conv（空间建模）
3. Cross-Modal Interaction: Node ↔ Edge 双向交互（V5.2 新增）
4. Host Causal Attention: 跨主机因果融合
5. Temporal Attention: Multi-head Attention（时序建模）
6. 对比学习: 投影头 + SupConLoss（V5.1）
7. 检测头: 重构 + 分类

损失函数:
    L = L_cls + λ_rec * L_rec + λ_causal * L_causal + λ_con * L_contrastive
"""
import torch
...
from models_msds.v5.contrastive import ContrastiveModule
from models_msds.v5.cross_modal import SimpleCrossModal
...
class MultiModalV5_MSDS(nn.Module):
    """
    MSDS V5 模型：对比学习增强
    ...
    def __init__(self, config: V5Config, adjacency_matrix: torch.Tensor = None):
        super().__init__()
        self.config = config
        ...
        # 2.5 Cross-Modal Interaction（V5.2 新增）
        self.use_cross_modal = config.use_cross_modal
        if self.use_cross_modal:
            self.cross_modal = SimpleCrossModal(
        ...
        self.disable_causal = config.disable_causal
        ...
        if self.use_contrastive:
            self.contrastive_module = ContrastiveModule(
                input_dim=config.embedding_dim * 2,  # node_feat 维度
    ...
    def _print_info(self):
        ...
        if self.use_contrastive:
            print(f"  对比学习: ✅ 启用 (τ={self.config.contrastive_temperature}, λ={self.contrastive_weight})")
        else:
            print(f"  对比学习: ❌ 禁用")
    ...
    def get_causal_matrix(self) -> torch.Tensor:
        """获取因果矩阵"""
        if self.disable_causal:
            return torch.eye(self.config.num_hosts)
    ...
    def forward(
        ...
        Returns:
            训练: (total_loss, rec_loss, cls_loss, causal_loss, contrastive_loss, causal_matrix)
        ...
        # ========== Step 2.5: Cross-Modal Interaction（V5.2 新增）==========
        if self.use_cross_modal:
            node_feat, edge_emb = self.cross_modal(node_feat, edge_emb, self.edge_indices)
            # 更新 edge_matrix
            edge_matrix = self._get_edge_attr_matrix(edge_emb)
        
        # ========== Step 3: Host Causal Attention ==========
        if self.disable_causal:
            causal_matrix = torch.eye(self.config.num_hosts, device=node_feat.device)
        else:
        ...
        # ========== Step 5: 对比学习（V5 新增）==========
        # 取最后时间步的特征用于对比学习
        node_feat_last = node_feat[:, -1, :, :]  # (B, N, 2D)
        
        if self.use_contrastive and not evaluate:
            # 获取标签（用于对比学习）
            # groundtruth_cls: (B, N, 3) -> labels: (B, N)
            # 0=正常, 1=异常, 2=unknown
            labels = groundtruth_cls.argmax(dim=-1)  # (B, N)
            
            # 创建 mask：只有正常(0)和异常(1)参与，unknown(2)不参与
            mask = (labels != 2).float()  # (B, N)
            
            # 将 unknown 的标签设为 0（不影响，因为 mask 会过滤）
            labels_for_contrast = labels.clone()
            labels_for_contrast[labels == 2] = 0
            
            contrastive_loss, _ = self.contrastive_module(
                node_feat_last, labels_for_contrast, mask
            )
        else:
            contrastive_loss = torch.tensor(0.0, device=node_feat.device)
        ...
        if evaluate:
            ...
            # 因果约束损失
            if self.disable_causal:
                causal_total = torch.tensor(0.0, device=total_mstgad.device)
            else:
            ...
            if not self.disable_causal:
                total_loss = total_loss + self.causal_loss_weight * causal_total
            if self.use_contrastive:
                total_loss = total_loss + self.contrastive_weight * contrastive_loss
            
            return total_loss, rec_loss, cls_loss, causal_total, contrastive_loss, causal_matrix
    ...
    def get_contrastive_embeddings(
        ...
        with torch.no_grad():
            ...
            if not self.disable_causal:
                node_feat, _ = self.host_causal(node_feat)
            
            for temporal_layer in self.temporal_layers:
                node_feat = temporal_layer(node_feat)
            
            # 取最后时间步
            node_feat_last = node_feat[:, -1, :, :]
            
            # 投影
            if self.use_contrastive:
                embeddings = self.contrastive_module.get_embeddings(node_feat_last)
            else:
                embeddings = node_feat_last
