"""
MSDS V4.4 模型：Graph Transformer 空间模块

在 V3 基础上：
- 用 Graph Transformer 替换 GATv2
- 添加图结构位置编码 (Random Walk PE)
- 全局注意力 + 边特征注入

架构：
1. Embedding: Metric/Log/Trace → D维 + PE
2. Spatial Attention: **Graph Transformer**（V4.4 核心改动）
...
from models_msds.v2.modules import TemporalAttentionBlock
from models_msds.v4.graph_transformer import GraphTransformerBlock
...
class MultiModalV4_4_GraphTransformer_MSDS(nn.Module):
    """
    MSDS V4.4 模型：Graph Transformer 空间模块
    
    与 V3 的区别：
    - V3: GATv2（局部消息传递，只看邻居）
    - V4.4: Graph Transformer（全局注意力 + 图结构位置编码）
    
    架构流程：
    1. Embedding: 三模态分别嵌入 + 时序 PE
    2. Spatial: **Graph Transformer**（全局注意力 + RWPE）
    ...
    def _print_info(self):
        ...
        print(f"V4.4 模型初始化完成（Graph Transformer 版）:")
        print(f"  嵌入维度: {self.config.embedding_dim}")
        print(f"  Graph Transformer 层数: {self.config.num_gat_layers}, heads: {self.config.gat_heads}")
        print(f"  位置编码: Random Walk PE (dim=16)")
    ...
    def forward(
        ...
        # ========== Step 2: Spatial Attention (Graph Transformer) ==========
