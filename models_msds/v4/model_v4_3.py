"""
MSDS V4.3 模型：Temporal Attention → Mamba

在 V3 基础上，用 Mamba (State Space Model) 替换 Multi-head Attention 做时序建模。
...
3. Host Causal Attention: 跨主机因果融合（继承自 V3）
4. Temporal: Mamba（V4.3 核心改动）
5. 检测头: 重构 + 分类
"""
import torch
...
class MultiModalV4_3_Mamba_MSDS(nn.Module):
    """
    MSDS V4.3 模型：Temporal Attention → Mamba
    ...
    def __init__(self, config: V4Config, adjacency_matrix: torch.Tensor = None):
        ...
        # 3. Host Causal Attention（继承自 V3）
    ...
    def _print_info(self):
        ...
        print(f"  跨主机因果: {'启用' if self.config.use_causal else '禁用'}")
    ...
    def forward(
        ...
        # ========== Step 3: Host Causal Attention（可选）==========
        if self.host_causal is not None:
            node_feat, causal_matrix = self.host_causal(node_feat)
        else:
            causal_matrix = None
        
        # ========== Step 4: Temporal (Mamba) ==========
